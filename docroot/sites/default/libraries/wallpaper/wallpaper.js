jQuery(document).ready(function(){  
  ...
  function setBackground(width) {
    ...
    if ( skin_large.length && width > 980 ) {
      $wallpaper.css('background', 'url("' + skin_large + '") no-repeat top center fixed');
      toggle_top_bg(0);
    }
    else if ( skin_tablet.length && width <= 980 && width >= 680 ) {
      $wallpaper.css('background', 'url("' + skin_tablet + '") no-repeat top center fixed');
      toggle_top_bg(0);
    }
    else if ( skin_mobile.length && width <= 680 && width >= 480) {
      $wallpaper.css('background', 'url("' + skin_mobile + '") no-repeat top center fixed');
      toggle_top_bg(0);
    }
    else {
