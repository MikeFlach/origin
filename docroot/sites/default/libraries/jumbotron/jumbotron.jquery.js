...
function jumbotronInit(){
	$jumboPanels = $('.jumbotron .panels ul');
	$jumboNav = $('.jumboNav ul');
...
function buildJumbotron(){
	// Reset jumbotron
	$(".jumboNav").html('');
	$(".jumbotron .nextprev, .jumbotron .navtitle, .jumbotron #navIndicator").remove();
	currentPanel=0;
	...
	strNav += "</ul>";
	strNav += "<div class=\"jumboNavNext\"><a href=\"#\" onclick=\"jumboClick('+');return false;\"><img src=\"images/jumbotron_nav_next.png\" /></a></div>";
	
	$(".jumboNav").html(strNav);
...
function jumboClick(dir,fromTimer){
	if(!animating){
		...
		var $nextPanel=$("#jpanel_"+nextPanel);
...
function jumboAnimate(dir){
	...
	switch (dir) {
		case '+':
			...
			$(".jumbotron #navIndicator").fadeOut(400,function(){ $(this).attr('class',''); });
			
			//clone non-cloned first item and put it as last item
			$jumboNav.children().last().after($("li:not(.clone)", $jumboNav).eq(0).clone());
			$("li:not(.clone)", $jumboNav).eq(0).addClass('clone');
			//slide the item
			$jumboNav.stop(true,true).animate({'left' : navLeftIndent}, animateTime, function () {
				...
				//set the default item to correct position
				$jumboNav.css({'left' : jumboNavLeftValue});
				$(".jumbotron #navIndicator").fadeIn(200).addClass('selector_arrow_'+currentPanel);
			});
		break;
		case '-':
			//get the right position
			...
			$jumboNav.stop(true,true).animate({'left' : navLeftIndent}, animateTime, function () {
				...
				$("li", $jumboNav).first().removeClass('clone');
				//clone 2nd to last item and put it as last item
				$jumboNav.children().first().before($("li", $jumboNav).eq(arJumbotron.length-1).clone().addClass('clone'));
				//remove last element
				$("li", $jumboNav).last().remove();
				//set the default item to correct position
				$jumboNav.css({'left' : jumboNavLeftValue});
